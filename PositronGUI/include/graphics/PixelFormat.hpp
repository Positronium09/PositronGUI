...
namespace PGUI::Graphics
{
	...
	struct PixelFormat
	{
		...
		explicit(false) PixelFormat(const D2D1_PIXEL_FORMAT& pf) noexcept :
			alphaMode{ static_cast<AlphaMode>(pf.alphaMode) },
			dxgiFormat{ static_cast<DXGIFormat>(pf.format) }
		{
		}
		PixelFormat(D2D1_ALPHA_MODE alphaMode, DXGI_FORMAT dxgiFormat) noexcept :
			alphaMode{ static_cast<AlphaMode>(alphaMode) },
			dxgiFormat{ static_cast<DXGIFormat>(dxgiFormat) }
		{
		}
		PixelFormat(AlphaMode alphaMode, DXGIFormat dxgiFormat) noexcept :
			alphaMode{ alphaMode },
			dxgiFormat{ dxgiFormat }
		{
		}

		explicit(false) operator D2D1_PIXEL_FORMAT() const noexcept
		{
			return D2D1::PixelFormat(static_cast<DXGI_FORMAT>(dxgiFormat), static_cast<D2D1_ALPHA_MODE>(alphaMode));
		}
	};
}
